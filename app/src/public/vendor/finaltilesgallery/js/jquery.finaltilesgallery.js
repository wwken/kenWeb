...
(function($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    setCurrentGridSize: function() {
      if (this.currentWidth < this.settings.disableGridSizeBelow)
        ...
        this.currentGridSize =
          this.settings.gridSize * this.currentImageSizeFactor;
    ...
    init: function() {
      ...
      if (filter_url) current_filter = filter_url;
        ...
        $(window).scroll(function() {
          if (!instance.ajaxComplete && !instance.isLoading) {
            if (instance.tiles.last().visible()) {
              instance.isLoading = true;
              if (instance.settings.onLoading) instance.settings.onLoading();

              $.get(
                instance.settings.autoLoadURL,
                { page: ++instance.currentPage },
                function(html) {
                  if (instance.settings.onUpdate) instance.settings.onUpdate();
    ...
    refresh: function() {
      this.$element.find('.edge').remove();
      this.edges = [{ left: 0, top: 0, width: this.currentWidth }];
    ...
    getBusyRowSpace: function() {
      ...
      for (var i = 0; i < this._rows[this._currentRow].length; i++) {
        space +=
          this._rows[this._currentRow][i].data('width') + this.settings.margin;
      }
      return space;
    },
    ...
    fitImagesInRow: function() {
      ...
      var ratio =
        (this.currentWidth -
          (this._rows[this._currentRow].length - 1) * this.settings.margin) /
      ...
      switch ($item.get(0).tagName.toLowerCase()) {
        case 'img':
          ...
          img.onload = function() {
            ...
            if ($tile.data('ftg-ignore-size-factor')) iFactor = 1;
    ...
    alignEdge: function(edge, index) {
      ...
      for (var i = 0; i < this.edges.length; i++) {
        if (
          this.edges[i].left - this.settings.margin ==
          edge.left + edge.width
        ) {
    ...
    add: function(tileIndex) {
      ...
      this.print(
        tileIndex + ' [' + $t.data('ftg-type') + '] (' + w + 'x' + h + ')'
      );
      ...
      if (hEdge.width < w + this.settings.margin) {
        ...
        if (
          this.settings.layout == 'columns' ||
          hEdge.width - w >= this.settings.minTileWidth
        ) {
          hEdge.case = 'tE';
          ...
          var m =
            hEdge.left + hEdge.width == this.currentWidth
              ? 0
              : this.settings.margin;
          //var w2 = hEdge.width - m;
          var w2 = hEdge.width;
          var h2 =
            this.settings.allowEnlargement && this.settings.layout != 'rows'
              ? h / w * w2
              : h;
      ...
      if ($t.data('ftg-type') != 'iframe') $item.css({ height: 'auto' });
  ...
  $(function() {
    $('.ftg-social a').click(function(e) {
      ...
      if (!image) image = $tile.find('.item').attr('src');

      var text = $.trim($tile.find('.subtitle').text());
      if (!text.length) text = document.title;

      var desc = $.trim($tile.find('.title').text());
      if (!desc.length) desc = document.title;
      ...
      if (social == 'facebook') {
        var url =
          ...
          '&' +
          'picture=' +
          encodeURIComponent(qualifyURL(image)) +
          '&' +
          'ref=share&' +
          'actions={%22name%22:%22View%20the%20gallery%22,%20%22link%22:%22' +
          encodeURIComponent(location.href) +
          '%22}&' +
