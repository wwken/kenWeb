!(function(i) {
  ...
  isNaN(n) || 0 >= n
    ? ((i.fn.carousel.Constructor.TRANSITION_DURATION = 1e3),
      i('.carousel-inner > .item').css({
        '-webkit-transition-duration': n + '1000ms',
        '-moz-transition-duration': n + '1000ms',
        'transition-duration': n + '1000ms',
      }))
    : ((i.fn.carousel.Constructor.TRANSITION_DURATION = n),
      i('.carousel-inner > .item').css({
  ...
  a &&
    i('.mouse_wheel_xy').bind('mousewheel', function(s) {
      s.originalEvent.wheelDelta / 120 > 0
        ? i(this).carousel('prev')
        : i(this).carousel('next');
    }),
    e &&
  ...
  h &&
    i('.thumb_scroll_y').on('slid.bs.carousel', function() {
      var s =
          -1 *
            i('.thumb_scroll_y .carousel-indicators li:first').position().top +
          i('.thumb_scroll_y .carousel-indicators li:last').position().top +
          i('.thumb_scroll_y .carousel-indicators li:last').height(),
        t =
          i('.thumb_scroll_y .carousel-indicators li.active').position().top +
          i('.thumb_scroll_y .carousel-indicators li.active').height() / 1,
        o = t + c,
        n = o - i('.thumb_scroll_y .carousel-indicators').height() / 1;
      0 > n && (n = 0),
        n > s - i('.thumb_scroll_y .carousel-indicators').height() &&
          (n = s - i('.thumb_scroll_y .carousel-indicators').height()),
    ...
    d &&
      i('.thumb_scroll_x').on('slid.bs.carousel', function() {
        var s =
            -1 *
              i('.thumb_scroll_x .carousel-indicators li:first').position()
                .left +
            i('.thumb_scroll_x .carousel-indicators li:last').position().left +
            i('.thumb_scroll_x .carousel-indicators li:last').width(),
          t =
            i('.thumb_scroll_x .carousel-indicators li.active').position()
              .left +
            i('.thumb_scroll_x .carousel-indicators li.active').width() / 1,
          o = t + u,
          n = o - i('.thumb_scroll_x .carousel-indicators').width() / 1;
        0 > n && (n = 0),
          n > s - i('.thumb_scroll_x .carousel-indicators').width() &&
            (n = s - i('.thumb_scroll_x .carousel-indicators').width()),
    ...
    i('.six_coloumns .item').each(function() {
      for (var s = i(this), t = 1; 6 > t; t++)
        (s = s.next()),
          s.length || (s = i(this).siblings(':first')),
          s
