...
(function($) {
  ...
  var $paradiseSlider = $('.carousel'),
    $firstAnimatingElems = $paradiseSlider
      .find('.item:first')
  ...
  if (thumbnailScrollY) {
    $('.thumb_scroll_y').on('slid.bs.carousel', function() {
      var heightEstimate =
        -1 * $('.thumb_scroll_y .carousel-indicators li:first').position().top +
        $('.thumb_scroll_y .carousel-indicators li:last').position().top +
        $('.thumb_scroll_y .carousel-indicators li:last').height();
      var newIndicatorPositionY =
        $('.thumb_scroll_y .carousel-indicators li.active').position().top +
        $('.thumb_scroll_y .carousel-indicators li.active').height() / 1;
      var toScrollY = newIndicatorPositionY + indicatorPositionY;
      var adjustedScrollY =
        toScrollY - $('.thumb_scroll_y .carousel-indicators').height() / 1;
      if (adjustedScrollY < 0) adjustedScrollY = 0;
      if (
        adjustedScrollY >
        heightEstimate - $('.thumb_scroll_y .carousel-indicators').height()
      )
        adjustedScrollY =
          heightEstimate - $('.thumb_scroll_y .carousel-indicators').height();
      $('.thumb_scroll_y .carousel-indicators').animate(
  ...
  if (thumbnailScrollX) {
    $('.thumb_scroll_x').on('slid.bs.carousel', function() {
      var widthEstimate =
        -1 *
          $('.thumb_scroll_x .carousel-indicators li:first').position().left +
        $('.thumb_scroll_x .carousel-indicators li:last').position().left +
        $('.thumb_scroll_x .carousel-indicators li:last').width();
      var newIndicatorPositionX =
        $('.thumb_scroll_x .carousel-indicators li.active').position().left +
        $('.thumb_scroll_x .carousel-indicators li.active').width() / 1;
      var toScrollX = newIndicatorPositionX + indicatorPositionX;
      var adjustedScrollX =
        toScrollX - $('.thumb_scroll_x .carousel-indicators').width() / 1;
      if (adjustedScrollX < 0) adjustedScrollX = 0;
      if (
        adjustedScrollX >
        widthEstimate - $('.thumb_scroll_x .carousel-indicators').width()
      )
        adjustedScrollX =
          widthEstimate - $('.thumb_scroll_x .carousel-indicators').width();
      $('.thumb_scroll_x .carousel-indicators').animate(
        { scrollLeft: adjustedScrollX },
        800
      );
