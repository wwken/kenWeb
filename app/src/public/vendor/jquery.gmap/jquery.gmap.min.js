...
(function(j) {
  ...
  p.prototype.dist = function(a) {
    return Math.sqrt(
      Math.pow(this.markers[0].latitude - a.latitude, 2) +
        Math.pow(this.markers[0].longitude - a.longitude, 2)
    );
  };
  ...
  p.prototype.getMarker = function() {
    ...
    var a, b;
    1 < this.markers.length
      ? ((a = new e.MarkerImage(
          'http://thydzik.com/thydzikGoogleMap/markerlink.php?text=' +
            this.markers.length +
            '&color=EF9D3F'
        )),
        (b = 'cluster of ' + this.markers.length + ' markers'))
      : ((a = new e.MarkerImage(this.icon)), (b = this.markers[0].title));
    return (this.mainmarker = new e.Marker({
      position: new e.LatLng(
  ...
  var e = google.maps,
    ...
    f = {
      init: function(a) {
        var b,
          c = j.extend({}, j.fn.gMap.defaults, a);
        for (b in j.fn.gMap.defaults.icon)
          c.icon[b] || (c.icon[b] = j.fn.gMap.defaults.icon[b]);
        return this.each(function() {
          var a = j(this),
            b = f._getMapCenter.apply(a, [c]);
          'fit' == c.zoom &&
            ((c.zoomFit = !0), (c.zoom = f._autoZoom.apply(a, [c])));
          var g = {
            zoom: c.zoom,
            center: b,
          ...
          c.controlsPositions.zoom &&
            (g.zoomControlOptions.position = c.controlsPositions.zoom);
          c.controlsPositions.pan &&
            (g.panControlOptions.position = c.controlsPositions.pan);
          c.controlsPositions.scale &&
            (g.scaleControlOptions.position = c.controlsPositions.scale);
          c.controlsPositions.streetView &&
            (g.streetViewControlOptions.position =
              c.controlsPositions.streetView);
          ...
          c.clustering.enabled
            ? ((b = a.data('gmap')),
              ...
              e.event.addListener(g, 'bounds_changed', function() {
                f._renderCluster.apply(a, []);
              }))
      ...
      _onComplete: function() {
        ...
        if (0 !== l)
          ...
          if (f._delayedMode) {
            ...
            a.opts.zoomFit &&
              ((c = f._autoZoom.apply(this, [a.opts, !0])), a.gmap.setZoom(c));
          }
          a.opts.onComplete();
        }
      },
      ...
      _getBoundaries: function(a) {
        ...
        if (b) {
          for (c = 0; c < b.length; c += 1)
            b[c].latitude &&
              b[c].longitude &&
              (h > b[c].latitude && (h = b[c].latitude),
              d < b[c].longitude && (d = b[c].longitude),
              g > b[c].longitude && (g = b[c].longitude),
              e < b[c].latitude && (e = b[c].latitude),
              a.log && console.log(b[c].latitude, b[c].longitude, h, d, g, e));
          f._boundaries = {
            N: h,
            E: d,
            W: g,
      ...
      _getBoundariesFromMarkers: function() {
        ...
        if (a) {
          for (b = 0; b < a.length; b += 1)
            c > a[b].getPosition().lat() && (c = a[b].getPosition().lat()),
              h < a[b].getPosition().lng() && (h = a[b].getPosition().lng()),
              d > a[b].getPosition().lng() && (d = a[b].getPosition().lng()),
              g < a[b].getPosition().lat() && (g = a[b].getPosition().lat());
      ...
      _getMapCenter: function(a, b) {
        ...
        if (a.markers.length && ('fit' == a.latitude || 'fit' == a.longitude))
          return (
            (d = b
              ? f._getBoundariesFromMarkers.apply(this)
              : f._getBoundaries(a)),
            (c = new e.LatLng((d.N + d.S) / 2, (d.E + d.W) / 2)),
            a.log && console.log(b, d, c),
            c
          );
        ...
        if (a.address)
          return (
            q.geocode({ address: a.address }, function(b, c) {
              c === google.maps.GeocoderStatus.OK
                ? f._setMapCenter.apply(h, [b[0].geometry.location])
                : a.log &&
                  console.log(
                    'Geocode was not successful for the following reason: ' + c
                  );
            }),
            c
          );
        if (0 < a.markers.length) {
          ...
          g.address &&
            q.geocode({ address: g.address }, function(b, c) {
              c === google.maps.GeocoderStatus.OK
                ? f._setMapCenter.apply(h, [b[0].geometry.location])
                : a.log &&
                  console.log(
        ...
        if ((d = a.gmap.getBounds())) {
          ...
          for (d = 0; d < b.length; d += 1)
            b[d].latitude < g.lat() &&
              ...
              (k[k.length] = b[d]);
          h.log &&
            console.log('number of markers ' + k.length + '/' + b.length);
          ...
          for (d = 0; d < k.length; d += 1) {
            ...
            for (
              ...
              b < c.length &&
              !((e = c[b].dist(k[d])),
              e < m && ((g = b), h.clustering.fastClustering));
              b += 1
            );
            -1 === g
              ? ((b = new p()), b.addMarker(k[d]), (c[c.length] = b))
              : c[g].addMarker(k[d]);
          }
      ...
      _processMarker: function(a, b, c, h) {
        ...
        if (!b) {
          var j = {
            ...
            infoWindowAnchor: new e.Size(
              f.icon.infowindowanchor[0],
              f.icon.infowindowanchor[1]
            ),
          };
          b = new e.MarkerImage(j.image, j.iconSize, null, j.iconAnchor);
        }
        c ||
          (new e.Size(f.icon.shadowsize[0], f.icon.shadowsize[1]),
          (j && j.iconAnchor) ||
          ...
          ((c = {
            content:
              'string' === typeof a.html
                ? f.html_prepend + a.html + f.html_append
                : a.html,
            pixelOffset: a.infoWindowAnchor,
          }),
          f.log && console.log('setup popup with data'),
          f.log && console.log(c),
          (n = new e.InfoWindow(c)),
          e.event.addListener(k, 'click', function() {
        ...
        a.html &&
          ...
          (d.infoWindow = n));
        a.onDragEnd &&
          e.event.addListener(k, 'dragend', function(b) {
      ...
      _geocodeMarker: function(a, b, c) {
        ...
        q.geocode({ address: a.address }, function(d, g) {
          g === e.GeocoderStatus.OK
            ? ((l -= 1),
              h.data('gmap').opts.log &&
                console.log(
                  'Geocode was successful with point: ',
                  d[0].geometry.location
                ),
              f._processMarker.apply(h, [a, b, c, d[0].geometry.location]))
            : (g === e.GeocoderStatus.OVER_QUERY_LIMIT &&
                (!h.data('gmap').opts.noAlerts &&
                  0 === r &&
                  alert(
              ...
              h.data('gmap').opts.log &&
                console.log(
                  'Geocode was not successful for the following reason: ' + g
                ));
        });
      },
      ...
      addMarker: function(a) {
        ...
        var c = b.icon.image,
          ...
          g = new e.Size(
            b.icon.infowindowanchor[0],
            b.icon.infowindowanchor[1]
          ),
          j = b.icon.shadow,
        ...
        a.icon &&
          (a.icon.image && (c = a.icon.image),
          a.icon.iconsize &&
            (h = new e.Size(a.icon.iconsize[0], a.icon.iconsize[1])),
          a.icon.iconanchor &&
            (d = new e.Point(a.icon.iconanchor[0], a.icon.iconanchor[1])),
          a.icon.infowindowanchor &&
            new e.Size(a.icon.infowindowanchor[0], a.icon.infowindowanchor[1]),
          a.icon.shadow && (j = a.icon.shadow),
          a.icon.shadowsize &&
            (k = new e.Size(a.icon.shadowsize[0], a.icon.shadowsize[1])),
          a.icon.shadowanchor &&
            (m = new e.Point(a.icon.shadowanchor[0], a.icon.shadowanchor[1])));
        c = new e.MarkerImage(c, h, null, d);
        j = new e.MarkerImage(j, k, null, m);
        a.address
          ? ('_address' === a.html && (a.html = a.address),
            '_address' == a.title && (a.title = a.address),
            b.log && console.log('geocoding marker: ' + a.address),
            (l += 1),
            (f._delayedMode = !0),
            f._geocodeMarker.apply(this, [a, c, j]))
          : ('_latlng' === a.html && (a.html = a.latitude + ', ' + a.longitude),
            '_latlng' == a.title && (a.title = a.latitude + ', ' + a.longitude),
            (b = new e.LatLng(a.latitude, a.longitude)),
            f._processMarker.apply(this, [a, c, j, b]));
      },
      ...
      changeSettings: function(a) {
        ...
        (a.latitude || a.longitude) &&
          b.gmap.panTo(f._getMapCenter.apply(this, [a]));
      },
      mapclick: function(a) {
        google.maps.event.addListener(this.data('gmap').gmap, 'click', function(
          b
        ) {
          a(b.latLng);
        });
      ...
      getRoute: function(a) {
        var b = this.data('gmap'),
          ...
          m = null,
          n = null;
        void 0 !== a.routeDisplay
          ? (k =
              a.routeDisplay instanceof jQuery
                ? a.routeDisplay[0]
                : 'string' == typeof a.routeDisplay
                  ? j(a.routeDisplay)[0]
                  : null)
          : null !== b.opts.routeFinder.routeDisplay &&
            (k =
              b.opts.routeFinder.routeDisplay instanceof jQuery
                ? b.opts.routeFinder.routeDisplay[0]
                : 'string' == typeof b.opts.routeFinder.routeDisplay
                  ? j(b.opts.routeFinder.routeDisplay)[0]
                  : null);
        f.setMap(c);
        null !== k && f.setPanel(k);
        m =
          void 0 !== g[b.opts.routeFinder.travelMode]
            ? g[b.opts.routeFinder.travelMode]
            : g.BYCAR;
        n =
          void 0 !== l[b.opts.routeFinder.travelUnit]
            ? l[b.opts.routeFinder.travelUnit]
            : l.KM;
        d.route(
          { origin: a.from, destination: a.to, travelMode: m, unitSystem: n },
          function(a, c) {
            c == e.DirectionsStatus.OK
              ? f.setDirections(a)
              : null !== k && j(k).html(b.opts.routeFinder.routeErrors[c]);
          }
        );
        return this;
      },
  ...
  j.fn.gMap.defaults = {
    ...
    routeFinder: {
      ...
      routeErrors: {
        ...
        OVER_QUERY_LIMIT:
          'A temporary error occured. Please try again in a few minutes.',
