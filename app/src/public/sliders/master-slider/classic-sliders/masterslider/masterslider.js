...
(function() {
  ...
  p._computeDeceleration = function() {
    if (this.options.snapping && this.__needsSnap) {
      var xtr_move =
        (this.__startSpeed - this.__speed) /
        this.__startSpeed *
        this.__extraMove;
...
(function($, window, document, undefined) {
  ...
  p._checkForShow = function(layer) {
    ...
    return (
      !slideId ||
      !layerHideOn ||
      (layerHideOn.length && layerHideOn.indexOf(slideId) === -1)
    );
  };
...
(function($) {
  ...
  p.create = function() {
    ...
    if (this.$element.data('action') !== undefined) {
      ...
      this.$element
        .on(this.$element.data('action-event') || 'click', function(event) {
  ...
  p.init = function() {
    ...
    for (var i = 0, l = this.__cssConfig.length; i < l; i++) {
      ...
      if (
        this.layerOrigin &&
        ((key === 'top' &&
          this.layerOrigin.indexOf('t') === -1 &&
          this.layerOrigin.indexOf('m') === -1) ||
          (key === 'bottom' && this.layerOrigin.indexOf('b') === -1) ||
          (key === 'left' &&
            this.layerOrigin.indexOf('l') === -1 &&
            this.layerOrigin.indexOf('c') === -1) ||
  ...
  p.locate = function() {
    ...
    for (var key in this.baseStyle) {
      ...
      if (key === 'top' && this.middleAlign) {
        ...
        isSize &&
        ((key === 'width' && this.maskWidth) ||
          (key === 'height' && this.maskHeight))
      ) {
        $layerEle[0].style[key] = this.baseStyle[key] * factor + 'px';
      } else {
  ...
  p.__updateSlides = function(slide, distance) {
    ...
    if (1 - value <= 0) {
      ...
      element.style[window._jcsspfx + 'Transform'] =
        ...
        this._rotateDir +
        '(' +
        value * (distance < 0 ? 1 : -1) * 90 * this._calcFactor +
        'deg)';
...
(function($) {
  ...
  p._resize = function() {
    ...
    this.bar_dimen =
      this.slider.api.view['__' + this.__dimen] *
      this.vdimen /
